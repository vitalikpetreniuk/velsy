...
// github.com/igorescobar/jQuery-Mask-Plugin
    ...
    function contentHeight(){
        ...
        section.css('min-height', $(window).outerHeight() - headerHeight - footerHeight);
    }
    contentHeight();
    ...
    $('.product-choose button').on('click', function () {
        var productIndex = $(this).index() + 1;
        $('.product-choose button:nth-child('+productIndex+')').addClass('active').siblings().removeClass('active');
    });

    var inputVal = false;
    var inputCheck = false;

    $('.form-check input, .form-check textarea').each(function () {
        if($(this).attr('required')){
            ...
            $(this).on('change', function () {
                ...
                $(this).closest('.section2').find('.required input, .required textarea').each(function () {
                    if(!$(this).val() || inputVal === false){
                        inputCheck = false;
                        return false;
                    }else{
                        inputCheck = true;
                ...
                if(inputCheck === false){
                    $(this).closest('section').find('.btn.next').addClass('disabled');
                    return false;
                }else{
                    $(this).closest('section').find('.btn.next').removeClass('disabled');
                }
            })
        }
    });
    ...
    $('.rate button').on('click touchstart', function () {
        console.log(1);
        rate = true;
        $(this).addClass('active').siblings().removeClass('active');
        $(this).closest('.section4').find('.required input, .required textarea').each(function () {
            if(!$(this).val() || rate === false){
                inputCheck2 = false;
                return false;
            }else{
                inputCheck2 = true;
    ...
    $('.form-block input, .form-block textarea').each(function () {
       if($(this).attr('required')){
           $(this).parent().addClass('required');
           $(this).on('input', function () {
               ...
               $(this).closest('.section6').find('.required input, .required textarea').each(function () {
                    if(!$(this).val()){
                        $(this).closest('section').find('.btn.next').addClass('disabled');
                        return false;
                    }else{
    ...
    $('.review-type textarea').on('input', function () {
        if(!$(this).val() == '') {
            $('#review').text($(this).val());
        }else{
            $('#review').text('nothing copied');
        }
    });

    $('section .btn.disabled').on('click', function () {
       $(this).closest('section').find('.required input, .required textarea').each(function () {
           if($(this).val() == '') {
               $(this).parent().addClass('error');
           }
       });
       $(this).closest('section').find('.required input[type="checkbox"]').each(function () {
           if(!$(this).is(':checked')) {
               $(this).parent().addClass('error');
           }
       });
    ...
    $('.to-amazon').on('click', function () {
        $('.section3 .btn.next').removeClass('disabled');
    });

    $(document).on('click', '.btn.next:not(.disabled)', function () {
        $(this).closest('section').removeClass('active').next().addClass('active');
    });

    $('.btn.copy').on('click', function () {
        select_all_and_copy(document.getElementById('review'));
        $(this).addClass('copied');
    });

    $('.characters').prevAll('textarea').on('input', function () {
        var maxlength = $(this).attr('maxlength');
        console.log(maxlength);
        $('.characters span').text(maxlength - $(this).val().length);
    });
    ...
    $('.post-code input').mask("00000");
    $('.order-num input').mask("000-0000000-0000000");
